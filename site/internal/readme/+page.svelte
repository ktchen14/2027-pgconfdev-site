<script lang="ts">
  import { Calendar, Check, Circle, Clock, MapPin } from "@lucide/svelte";
...
<svelte:head>
  <title>CSS utilities — PGConf.dev 2027</title>
...
<main>
  <nav class=") none@-48 note" aria-labelledby="on-this-page">
    ...
    <ol>
      <li><a class="li-number" href="#typography">Typography</a></li>
  ...
  <section id="typography">
    <div class="main p">
      <hgroup>
        <h2 class="section-number">Typography</h2>
        <p>Set the font, size, and emphasis of text.</p>
      </hgroup>

      <h3>Headings, labels, and summaries</h3>

      <p>
        An <T>&lt;hgroup&gt;</T> ties a heading together with secondary text that
        titles it, instead of leaving that text as a separate paragraph.
      </p>

      <p>
        A paragraph <em>before</em> the heading acts as a label that categorizes
        it, i.e. the section or kind of thing the heading belongs to. A
        paragraph
        <em>after</em> it acts as a summary or subtitle.
      </p>

      <p>
        The role comes from position, so you add no classes inside an
        <T>&lt;hgroup&gt;</T>.
      </p>
...
<p>
  Body text after the group, shown at the normal size for comparison.
      ...
      <p>
        The same two roles are available as classes for framing content that
        isn't a heading, such as a figure or statistic. <T>.over</T> is a label that
        introduces what follows, and <T>.deck</T> is a summary subordinate to the
        text above it.
      </p>
...
<p class="deck">From 320 backers in the first week.</p>`}
      ...
      <h3>Lead paragraphs and code</h3>

      <p>
        <T>.lead</T> (or <T>.lede</T>) marks the opening paragraph of a section,
        i.e. the sentence or two that introduce or summarize what follows. It
        gives that paragraph extra weight to match its role.
      </p>
...
<p>
  The first day centers on scheduled talks; the remaining two open up to unconference sessions and hallway-track hacking.
      ...
      <p>
        <T>.mono</T> sets text in the monospace typeface. Use it only for text that
        isn't code as <code>&lt;code&gt;</code>, <code>&lt;kbd&gt;</code>, and
        <code>&lt;samp&gt;</code> already render monospace on their own. Good cases
        are a label or a run of figures you want aligned.
      </p>
...
<p>
  Doors open at <span class="mono">09:00</span> — find us at booth <span class="mono">B12</span>.
      ...
      <p>
        ...
        <code>&lt;blockquote&gt;</code>s), while leaving headings and other
        elements untouched.
      </p>
...
<div class="note">
  ...
  <ul>
    ...
    <li>The heading above keeps its normal size.</li>
      ...
      <h3 id="typography-size">Size</h3>

      <p>
        The size scale changes how large text renders relative to its
        surroundings without changing what the text means — a purely
        presentational adjustment. Reach for it only when no semantic element
        like <T>.lead</T>, <T>.deck</T>, or a heading already carries the
        meaning you want.
      </p>
...
<p class="size--">Two steps smaller</p>`}
  ...
  <section id="color" class="main">
    <hgroup>
      ...
      <p>Carry meaning with the color of text.</p>
    </hgroup>
...
<p class="mute">Muted text steps back for secondary detail.</p>`}
    />
...
<p class="delete">
  Delete, for errors and removals.
    ...
    <p>
      Each class also defines a matching muted shade, so <T>.mute</T> text nested
      inside takes on the same hue instead of neutral gray.
    </p>
...
<p class="action">
  ...
  <section id="icons" class="main">
    ...
    <p>
      ...
      <a href="https://lucide.dev/icons/" target="_blank">gallery</a>, convert
      its name to PascalCase (e.g. <code>map-pin</code> → <code>MapPin</code>),
      import it from <code>"@lucide/svelte"</code>, and drop it in as a Svelte
      component.
    </p>
...
<MapPin />`}
    ...
    <p>
      <T>.iconic</T> lays an icon and a label out on one line, vertically centered
      with a small gap.
    </p>
...
<p class="iconic">
  ...
  <span>Ticket Confirmed</span>
    ...
    <p>
      ...
      <a href="#color">semantic color classes</a> and the size scale apply to them
      just as they do to text.
    </p>
...
<a class="button" href="#">Learn more</a>`}
    ...
    <p>
      <T>.stroke</T> gives a button an outlined look instead of a filled one, for
...
<button class="stroke">Learn more</button>`}
    ...
    <p>
      <T>.circle</T> gives a button fully rounded corners, for pill-shaped or
...
<p>
  ...
  <button class="stroke delete">Delete</button>
    ...
    <p>
      <T>.iconic</T> composes naturally with buttons, placing an icon and label on
      one line.
    </p>
...
<button class="circle iconic stroke">
  ...
  <section id="areas" class="main">
    ...
    <p>
      <T>.area</T> fills a block with the neutral tinted surface and adds padding,
      setting a group of content apart from the page without a hard border. Use it
      for cards, asides, and banded sections.
    </p>
...
<p class="area">
  Everything here reads as one unit, lifted off the page by a soft fill.
    ...
    <p>
      <T>.border</T> outlines a block on all sides and adds the same padding; it's
      the unfilled counterpart to <T>.area</T>.
    </p>
...
<p class="border">
  An outlined box, the unfilled counterpart to an area.
    ...
    <p>
      <T>.band</T> brackets a block with a rule above and below, for a full-width
      strip that sets a run of content off without enclosing its sides.
    </p>
...
<p class="band">
  A band, bracketed by a rule above and below.
    ...
    <p>
      Each role from the color section has an area form that fills the block
      with that role's color, for callouts: <T>.action-area</T>, <T
        >.notice-area</T
      >,
      <T>.insert-area</T>, and <T>.delete-area</T>. Each one also retunes the
      fill and border colors inside, so a nested border or accent rule takes the
      same role.
    </p>
...
<p class="action-area border">
  ...
  <section id="accents" class="main*">
    ...
    <p>
      <T>.acme</T> puts the rule across the top. It suits a pull quote or the head
      of a section.
    </p>
...
<blockquote class="edge">
  ...
  <footer>
    ...
    <p>
      Prefix a role to either shape to color the rule, carrying the same
      meanings as the <a href="#color">color</a> section.
    </p>
...
<p class="delete-edge">Delete</p>`}
    ...
    <p>
      Accents compose with the areas from the previous section. Because each
      area resets the default accent color to its own role, a plain
      <T>.edge</T> on a <T>.notice-area</T> takes the area's color:
    </p>
